...
public final class CombatFactory {
	 ...
	 * @return true if the player is wearing full guthans.
	 */
	public static boolean DarkLord(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Darklord")
				: ((Player) entity).getEquipment().containsAll(3687, 3688, 3689);
	}
	/**
	 * Determines if the entity is wearing full tokTkzar.
	 * 
	 * @param entity
	 *            the entity to determine this for.
	 * @return true if the player is wearing full tokTkza.
	 */
	public static boolean tokTkzar(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"tokTkzar")
				: ((Player) entity).getEquipment().containsAll(15662, 15663, 15664,
						15665, 15666, 15667, 15668);
				
	}
	public static boolean FlamingWhip(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"FlamingWhip")
				: ((Player) entity).getEquipment().containsAll(17848);
				
	}
	public static boolean LavaGodsword(Character entity) {
		return entity.isNpc() ? ((NPC) entity).getDefinition().getName().equals(
				"Weapon")
				: ((Player) entity).getEquipment().containsAll(178478);
	...
	public static int calculateMaxMeleeHit(Character entity, Character victim) {
		...
		if (victim.isNpc()) {
			...
			if(npc.getId() == player.getSlayer().getSlayerTask().getNpcId()) {
				...
				if(player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 11546) {
					maxHit *= 1.45;
				}
				if(player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 11547) {
					maxHit *= 1.55;
				}
				if(player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 11548) {
					maxHit *= 1.65;
				}
				if(player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 11549) {
					maxHit *= 1.85;
				}
				if(player.getEquipment().getItems()[Equipment.HEAD_SLOT].getId() == 11550) {
					maxHit *= 1.75;
				}
			}
	...
	protected static void handleArmorEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		// 13% chance of armour effects happening
		if (RandomUtility.exclusiveRandom(1) == 0) {
		if (CombatFactory.FlamingWhip(attacker)) {
			target.performGraphic(new Graphic(1618));
			return;
		}
	}
		if (RandomUtility.exclusiveRandom(1) == 0) {
		if (CombatFactory.LavaGodsword(attacker)) {
			target.performGraphic(new Graphic(1618));
			return;
		}
	}
		if (RandomUtility.exclusiveRandom(6) == 0) {
		if (CombatFactory.tokTkzar(attacker)) {
			attacker.performGraphic(new Graphic(6));
			attacker.heal(damage);
			return;
		}
	}
		if (RandomUtility.exclusiveRandom(1) == 0) {
		if (CombatFactory.DarkLord(attacker)) {
			attacker.performGraphic(new Graphic(128));
			attacker.poisonVictim(target, CombatType.MELEE);
						return;
		}
		}
